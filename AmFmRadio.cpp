...
AmFmRadio::AmFmRadio(bool newOn)
{
	...
	for (int i = 0; i < 5; ++i)
	{
		...
		button[i].FMFreq = (float)MINIMUM_FM;
...
AmFmRadio::AmFmRadio(bool newOn, Freqs newButton[])
{
	...
	for (int i = 0; i < MAX_FREQS; i++)
	{
		...
		button[i].FMFreq = button[i].FMFreq;
		button[i].currentStation = MINIMUM_FM;
		button[i].volumeStruct = 0;
		displayOutPut = false;
	}
...
/*  -- Method Header Comment
	Name	: GetFrequency
	Purpose : This method will return a string contain a copy of frequency data member
	Inputs	:	NONE
	Outputs	:	NONE
...
{
	char copyStr[MAX_CHAR] = { 0 };
	strcpy(copyStr, frequency);
...
/*  -- Method Header Comment
	Name	: GetButton
	Purpose : This method will return the copy of `freqs` array 
	Inputs	:	NONE
	Outputs	:	NONE
